...
export default async function handler(req, res) {
  ...
  if (!parentId) {
    ...
    res.end(JSON.stringify({ success: false, error: 'parentId query param is required' })); return;
  }
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(parentId)) {
    res.writeHead(400, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ success: false, error: 'Invalid parentId format' })); return;
  }
  ...
  try {
    ...
    if (linksError) {
      res.writeHead(500, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ success: false, error: linksError.message })); return;
    }

    if (!links || links.length === 0) {
      res.writeHead(200, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ success: true, posts: [] })); return;
    }

    const childIds = links.map((l) => l.child_id);

    const { data: posts, error: postsError } = await supabase
      .from('post_queue').select('*').eq('parent_id', parentId).eq('status', 'pending').in('child_id', childIds).order('created_at', { ascending: false });
    if (postsError) {
      res.writeHead(500, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ success: false, error: postsError.message })); return;
    }
    ...
    const { data: childUsers, error: usersError } = await supabase
      .from('family_users').select('id, display_name, email').in('id', uniqueChildIds);
    if (usersError) {
      res.writeHead(500, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ success: false, error: usersError.message })); return;
    }

    const childMap = {};
    (childUsers || []).forEach((u) => { childMap[u.id] = { displayName: u.display_name, email: u.email }; });
